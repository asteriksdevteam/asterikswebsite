    ...
    <div class="row">
        <div class="col-xl-12">
            <section class="hk-sec-wrapper">
                ...
                <div class="row">
                    <div class="col-sm">
                        <form method="post" action="{{@$metas ? route('meta.update',['id'=>$metas->id] ) : route('meta.store') }}" class="" enctype="multipart/form-data">
                        @csrf
                        @if(@$metas)
                            @method('put')
                        @endif
                                    ...
                                    <div class="col-lg-6">
                                        <select class="form-control m-input page_id" name="page_slug" required>
                                         ...
                                         @foreach($pages as $cvalue)
                                            <option value="{{ $cvalue['slug'] }}" {{ $cvalue->id == @$blog->page_id ? 'Selected' : '' }}>{{ $cvalue['page_name'] }}
                                            </option>
                                            @endforeach
                                        </select>
                                    </div>
...
$('.page_id').on('change',function(e){
    ...
    $.ajax({
        type :'GET',
        url : '{{ url('meta/get-by-pageid') }}?slug='+slug,
